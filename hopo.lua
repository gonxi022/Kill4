...
local function scanRealFish()
    local fish = {}
    local minValue = 50 * 1000000000000 -- 50T
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Text and obj.Visible and obj.Parent then
            ...
            if num and text:find("T") and num >= 50 then
...
local function createGUI()
    ...
    local function updateFishList()
        ...
        fishCountLabel.Text = string.format("Peces 50T+: %d", #fish)
        ...
        if #fish == 0 then
            ...
            noFish.Text = "No hay peces de 50T+ en este servidor"
    ...
    -- Función para encontrar servidor con menos jugadores
    local function findBestServer()
        ...
        for _, server in ipairs(servers) do
            if server.id ~= game.JobId and server.playing < minPlayers and server.playing > 0 then
        ...
        if bestServer then
            fishCountLabel.Text = string.format("Mejor servidor: %d jugadores", bestServer.playing)
            TeleportService:TeleportToPlaceInstance(game.PlaceId, bestServer.id, LocalPlayer)
        else
            fishCountLabel.Text = "No se encontraron servidores mejores"
        end
    ...
    -- Conectar botones
    refreshBtn.MouseButton1Click:Connect(updateFishList)
    hopBtn.MouseButton1Click:Connect(serverHop)
    findBestBtn.MouseButton1Click:Connect(findBestServer)
...
-- Ejecutar
createGUI()
print("🐟 Real Fish Scanner cargado - Solo servidores REALES")